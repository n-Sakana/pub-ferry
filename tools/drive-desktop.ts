...
//
//   1. start the app:  pc\decimen.ps1 -DebugPort 9333
...
// mock-up: __decimen.cameraProblem() calls the same function a refused
// getUserMedia calls.
...
const port = Number(process.env.DECIMEN_DEBUG_PORT ?? 9333);
...
async function main(): Promise<void> {
  ...
  await page.evaluate(async () => {
    const api = window as unknown as {
      __decimen: { setForTest?: (s: Record<string, unknown>) => Promise<void> };
    };
    await api.__decimen.setForTest?.({
      deviceLabel: "事務所デスクトップ",
      destination: "D:/decimen/受信箱",
    });
  });
  ...
  await page.evaluate(async (folder: string) => {
    const api = window as unknown as {
      __decimen: { pickForTest(path: string): Promise<void> };
    };
    await api.__decimen.pickForTest(folder);
  }, sample);
  ...
  await page.evaluate(() => {
    (window as unknown as { __decimen: { cameraProblem(kind: string): void } }).__decimen.cameraProblem("none");
  });
  await settle(page);
  await capture("05-camera-none", "カメラなし（警告色にしない／リーダーへ誘導）");

  await page.evaluate(() => {
    (window as unknown as { __decimen: { cameraProblem(kind: string): void } }).__decimen.cameraProblem("denied");
  });
  await settle(page);
  ...
  await page.evaluate(() => {
    (window as unknown as { __decimen: { goto(name: string): void } }).__decimen.goto("receive-choose");
  });
  ...
  const frames = JSON.parse(process.env.DECIMEN_SLOW_FRAMES ?? "[]") as string[];
  if (frames.length > 0) {
  ...
  await page.evaluate(() => {
    (window as unknown as { __decimen: { goto(name: string): void } }).__decimen.goto("send-pick");
  });
  await settle(page);
  ...
  for (const [screen, label] of [
    ...
    await page.evaluate((name: string) => {
      (window as unknown as { __decimen: { goto(n: string): void } }).__decimen.goto(name);
    }, screen);
