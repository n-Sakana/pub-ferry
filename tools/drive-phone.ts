...
async function main(): Promise<void> {
  ...
  const root = mkdtempSync(join(tmpdir(), "pub-ferry-phone-"));
  ...
  // PUB_FERRY_CHROMIUM points at one that is already on the machine, so the
  // screen tests do not depend on a download.
  const executablePath = process.env.PUB_FERRY_CHROMIUM || undefined;
  ...
  for (const viewport of VIEWPORTS) {
    ...
    await page.evaluate(() => {
      (window as unknown as { __pubFerry: { cameraProblem(k: string): void } }).__pubFerry.cameraProblem("denied");
    });
