...
function findChromium(): string | null {
  if (process.env.DECIMEN_CHROMIUM && existsSync(process.env.DECIMEN_CHROMIUM)) {
    return process.env.DECIMEN_CHROMIUM;
  }
...
function reasonNotRun(): string {
  if (chromiumPath === null) {
    return "Chromium が見つかりません（DECIMEN_CHROMIUM に実行ファイルを指定してください）";
  }
...
before(async () => {
  ...
  const probeRoot = mkdtempSync(join(tmpdir(), "decimen-camera-probe-"));
  try {
  ...
  ready = true;
  root = mkdtempSync(join(tmpdir(), "decimen-optical-"));
