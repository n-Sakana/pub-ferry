...
async function boot(): Promise<void> {
  ...
  if (!isHosted()) {
    ...
    showFailure(
      ...
      ["pc\\pub-ferry.bat をダブルクリックして起動してください。"],
    );
    return;
  }
...
function applyStoredTheme(): void {
  const stored = localStorage.getItem("pub-ferry.theme");
...
function toggleTheme(): void {
  ...
  localStorage.setItem("pub-ferry.theme", next);
}
...
// DevTools protocol, which pub-ferry.ps1 opens only when asked.
