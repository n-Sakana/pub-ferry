...
`%LOCALAPPDATA%\PubFerry\relay\config.json`（Windows）または
`~/.config/pub-ferry/relay/config.json`（Linux）ができます。
...
```
npm run relay -- route add "受信箱" "D:\pub-ferry\受信箱"
...
{
  "acls": [
    { "action": "accept", "src": ["tag:phone"], "dst": ["tag:pub-ferry-host:8842"] }
  ]
}
```
...
sudo systemctl status pub-ferry-relay
```
